...
class base_utils(object):
    ...
    @staticmethod
    def cameraparam_opt(intrinsic, dist, extrinsic_list, imgpoints_list, objpoints_list,expect_rme=0.5, least_pic=10):
        """
        优化相机参数，包括相机内参和相机外参
        ...
        def camera_loss_function(P, discoff_len, imgpoints, objpoints_list):
            ...
            return base_utils.reprojection_error(intrisic, dist, extrinsic_list, imgpoints, objpoints_list)
    ...
    @staticmethod
    def RME_each_pic(intrisic, dist, extrinsic_list, imgpoints,objpoints_list):
        """
        计算每幅图片的重投影误差，用于减少误差
        :param intrisic:
        :param dist:
        ...
        return errors
    @staticmethod
    def intrinsic(imgpoints_list, objpoints_list,imgsize):
        """
        相机内参标定
        :param imgpoints_list: 图片角点列表
...
>>>>>>> a9a878cb43e13dffec3592cbab7fb57fda9eaf22
    ...
    @staticmethod
    def stereo_calibration(imgpoints_list1,objpoints_list1,imgpoints_list2,objpoints_list2,intrinsic1, dist1, intrinsic2, dist2,
        ...
        while(True):
            ...
            R = transforms3d.quaternions.quat2mat(q)
            H = np.append(np.append(R, np.transpose([t]), 1), np.array([[0, 0, 0, 1]]), 0)
            errors = base_utils.stereo_rme_mm_each_group(extrinsic_list1,extrinsic_list2,H,board_corner)
    ...
    @staticmethod
    def stereo_rme_mm_each_group(extrinsic_list1, extrinsic_list2, H, board_corner):
        if board_corner.shape[1] == 2:
            board_corner = np.append(board_corner, np.zeros([board_corner.shape[0], 1]), 1)
        if board_corner.shape[1] == 3:
            board_corner = np.append(board_corner, np.ones([board_corner.shape[0], 1]), 1)
        ...
        return errors
    @staticmethod
    def intrinsic_depth_opt(objPoints_list, imgPoints_list, depth_list, intrinsic, discoff):
        def distance_error(X, lengthdiscoff, objPoints_list, imgPoints_list, depth_list):
            ...
            for i in range(n):
                ...
                plane = base_utils.get_nice_plane(depth_point_acc)
                for j in range(depth_point_acc.shape[0]):
    ...
    @staticmethod
    def extrisic_depth(objpoint,imgpoint,point_depth,intrinsic,dist):
        ...
        plane = base_utils.get_nice_plane(point_in_camera)
